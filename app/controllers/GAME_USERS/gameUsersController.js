    ...
    if (
      ...
      if (
        ...
        res.json({
          error: true,
          message:
            "Invalid input: winning_ball should be between 0 and 15, excluding 8 and 9",
        });
      } else {
...
exports.getGameUserByGameId = async (req, res, next) => {
    ...
    //   game_details.commission = parseFloat(game_details.commission).toFixed(4);
      game_details.winning_amount = parseFloat(game_details.winning_amount).toFixed(2);
      game_details.winning_amount_single = parseFloat(game_details.winning_amount_single).toFixed(2);
      game_details.commision_winning_amount = parseFloat(game_details.commision_winning_amount).toFixed(2);
    ...
    //   game_details.winners = parseFloat(game_details.winners).toFixed(4);
      ...
      let participants_array = [];
      for (let i = 0; i < gameUsers.rows.length; i++) {
        const user_id = gameUsers.rows[i].user_id;
        const gameUsersDetails = await pool.query(
      ...
      if (game_status === "completed") {
        ...
        if (winner_ball == 0) {
          ...
          winnerCondition = "winning_ball IN ('1', '2', '3', '4', '5', '6', '7', '8')";
        } else if (winner_ball == 9) {
          winnerCondition = "winning_ball IN ('9', '10', '11', '12', '13', '14', '15')";
        } else {
      ...
      res.json({
        ...
        participated_users,
        balls_count: result,
        jackpot:Number(jackpot||0) % 1 === 0 ? Number(jackpot||0) : Number(jackpot||0).toFixed(2),
        participants_array,
        winners_array: winnersUsersArray,
        message: "Game Data Fetched Successfully",
      });
